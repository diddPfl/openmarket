...
export const AuthProvider = ({ children }) => {
  // isAuthenticated: 사용자가 인증된 상태인지 여부를 나타내는 상태 변수
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  // member: 로그인된 사용자의 이름과 역할을 저장하는 상태 변수
  const [member, setMember] = useState({ name: '', roles: [] }); // roles를 배열로 수정
  ...
  useEffect(() => {
    ...
    const storedRoles = sessionStorage.getItem('roles'); // roles 저장된 것 가져오기
  ...
  const login = (token, name, roles) => {
    ...
    sessionStorage.setItem('name', name);   // 사용자 이름 저장
    sessionStorage.setItem('roles', JSON.stringify(roles)); // roles을 JSON.stringify로 저장
    setIsAuthenticated(true);               // 인증 상태 업데이트
    setMember({ name, roles });              // member 상태 업데이트
  };

  // logout 함수: 사용자가 로그아웃할 때 호출됩니다.
  // 세션 스토리지에서 JWT 토큰과 사용자 정보를 제거하고,
  // 인증 상태와 사용자 정보를 초기화합니다.
  const logout = () => {
    sessionStorage.removeItem('jwt');   // JWT 토큰 제거
    sessionStorage.removeItem('name');  // 사용자 이름 제거
    sessionStorage.removeItem('roles');  // 사용자 역할 제거
    setIsAuthenticated(false);          // 인증 상태 초기화
    setMember({ name: '', roles: [] });  // member 상태 초기화
  };

  // AuthContext.Provider: AuthContext를 사용하여 전역 상태를 자식 컴포넌트에 제공합니다.
  // value로 전달되는 객체는 로그인 상태, 사용자 정보, 로그인 및 로그아웃 함수입니다.
  return (
